    ...
    <div class="create-channel">
        <el-form size="small" :model="createForm" :rules="createRules" ref="createForm" label-width="100px">
                <el-form-item prop="parent_channel_id" label="父渠道">
                    <label>{{createForm.market_channel.name}}</label>
                </el-form-item>
...
export default {
  ...
  mounted() {
    this.getMarketSubChannelById({
      ...
      if (res.code == 0) {
        ...
        // this.createForm.parent_channel_id = res.data.parent_channel_id;
  ...
  methods: {
    ...mapActions({
      updateMarketSubChannel: "updateMarketSubChannel",
      getMarketSubChannelByName: "getMarketSubChannelByName",
      getMarketSubChannelById: "getMarketSubChannelById"
    }),
    onSubmit() {
      this.$refs["createForm"].validate(valid => {
        if (valid) {
          this.updateMarketSubChannel(
            Object.assign(
              { school_id: this.current_school.id },
              this.createForm
            )
